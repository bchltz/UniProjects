...
In this step, we take a look at the distribution of our rentals on free/non-free days.
```{r}
table(train_data$freierTag)

283 / (283 + 642) # percentage of free days
```
30.6% of our days are free days.

```{r}
train_data %>%
  group_by(freierTag) %>%
  dplyr::summarize(rentals=sum(rentals))

1867011 / (1867011+4853468) # percentage of rentals on free days
```
27.8% of our rentals are on free days. This shows that there are a bit less rentals on free days.

Now we compare the mean duration (in minutes) for our category.
```{r}
train_data %>%
  group_by(freierTag) %>%
  dplyr::summarize(dauer=mean(dauer))
```
While there are less rentals on free days, the mean duration is higher by 5.3 minutes per rental.

Let´s have look at the distribution of our rentals depending on the temperature and divide the observations into free and non-free days. Our calculations above on the distribution seem to be evident here as well.
```{r}
train_data %>%
  ggplot(aes(x = Temperatur, y = rentals, color = freierTag)) +
  ggtitle(label = "Distribution of rentals (free and non-free)") +
  geom_point() +
  theme_light()
```

The mean temperatures of free and non-free days are (as expected) almost equal. The histograms of the temperature distribution of the rentals don´t show any differences at first glance as well.
```{r}
train_data %>%
  group_by(freierTag) %>%
...
While there are less rentals on free days, the duration per rent is higher (duration outliers are removed from the plot).
```{r}
# plot of rentals
...
# Modeling
In this chapter, we will build and train three models using the parsnip package: Logistic Regression with the glm engine, a Random Forest Model using the Ranger engine and a boosted tree model using XGBoost.
As the setup for all models is the same, we will explain the individual steps only for the Logistic Regression.


## K-fold cross-validation
Due to the amount of observations we prepare a 5-fold cross-validation
```{r}
set.seed(123)
...
Ergebnisse_class <- tibble(Model = "Logistische Regression", 
...
bike_fit_lr_mode %>%
  ...
  coord_equal()
```
The ROC and Accuracy values of the individual folds fluctuate visibly, as can be seen in the plot. The ROC values are between 0.778 and 0.951 with a mean ROC of 0.847, the Accuracy values are between 0.730 and 0.891 with a mean of 0.794. Considering that in the training data set we have only 925 observations distributed over 5 folds, the individual fold composition can indeed be very different. Another reason could be that our model is not very robust and reacts strongly to slight fluctuations.
...
final_model %>%
  ...
  coord_equal()
```
The area under the ROC curve (AUC) can be used as a summary of the model skill. In our final model, the value roc_auc is 0.955 which means that there is a chance of 95.5% that the model is able to distinguish between a free and a non-free day.
...
## Summary
As the results show, the performance of our XGBoost model on the test data is even better than on the training data in some parameters. But basically the results are very similar and therefore in our test it can be assumed that the model is robust and reliable.

As in our regression test it must be mentioned here as well that some of the predictors of our models are very difficult to predict itself. The prediction of weather data is generally very error-prone. Therefore, our model in the real world may be even less accurate than in our experiment with the test data. 
```{r}
